...
pub fn slice(mut what: String, len: String) -> String {
    // what.truncate(len as usize);
    if let Some((l, r)) = len.split_once(':') {
        let li = l.parse::<usize>().unwrap();
        let ri = if r.is_empty() { what.len() } else { r.parse::<usize>().unwrap() };
...
type SlicedInner =
TryYield<AndThenTry<StringGenerator, Box<dyn Fn(String) -> Slicer>, Slicer>>;
...
impl Sliced {
    pub(crate) fn new(content: StringGenerator, length: StringGenerator) -> Self {
